...
// بدلاً من تكراره هنا — يستخدمه أيضاً pages/api/teacher/content.js وcron المستقبلي للحذف المجدول.
import {
  deepDeleteCourse,
  getVideoRowsUnder,
    ...
    try {
      ...
      if (action === 'delete') {
         ...
         if (type === 'courses') {
            ...
            // (نفس المنطق المستخدم في pages/api/teacher/content.js وcron الحذف المجدول)
            ...
            if (type === 'videos') {
                ...
                // 👈 حذف مادة أو فصل يحذف عبره فيديوهاته (Cascade) — نجمعها أولاً لتنظيف Bunny وFirestore لاحقاً
         ...
         // (بعد استبعاد أي بunny_video_id لا يزال مستخدَماً في صف آخر — راجع getBunnyIdsSafeToDelete)
         ...
         // في الخلفية دون انتظار النتيجة (لا تؤثر على استجابة العميل)
