...
export async function requestOtp({ email, purpose = 'signup' }) {
  ...
  // نمسح أي صفوف قديمة لنفس البريد+الغرض قبل إدخال صف جديد، عشان الجدول
  // ما يتراكمش (كان بيخلي البحث في verifyOtp/consumeVerifyToken يبطأ
  // تدريجياً كل ما الجدول يكبر).
  await supabase
    .from('otp_verifications')
    .delete()
    .eq('email', email)
  ...
  //
  // ⚡ متعمدين عدم انتظار (await) اكتمال الإرسال هنا. الكود بقى مسجل في
  // الداتابيز فعلياً (ده مصدر الحقيقة الوحيد)، فبنرجّع النجاح للكلاينت على
  // طول من غير ما ننتظر الـ SMTP relay اللي ممكن ياخد من ثانيتين لدقايق.
  // لو الإرسال فشل أو اتأخر، المستخدم لسه يقدر يعمل "إعادة إرسال" بعد
  // الـ cooldown، والفشل بيتسجل في الـ server logs (console.error) عشان
  // نراقبه. الأمان هنا معتمد على إن السيرفر VPS (عملية Node مستمرة)، مش
  // serverless — فالعملية مش بتتقفل بعد ما الريكوست يخلص.
  const { text, html } = emailTemplate(code);
  sendMail({ to: email, subject: 'رمز التحقق - Medaad', text, html }).catch((mailError) => {
